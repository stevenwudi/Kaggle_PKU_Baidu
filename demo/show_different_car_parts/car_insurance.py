...
def plotly_trisurf(x, y, z, simplices, left_front_max_x, left_front_min_x, left_front_max_y, left_front_min_y, left_front_max_z, left_front_min_z, colormap=cm.RdBu, plot_edges=None):
    ...
    for i, tri_i in enumerate(I_pt):
        if left_front_min_x < xmean[i] < left_front_max_x:
            # point = [ymean[i], zmean[i]]
            # if isInsidePolygon(point, pt_y_z):
                # print(isInsidePolygon(point, pt_y_z))
            if left_front_min_y < ymean[i] < left_front_max_y:
                if left_front_min_z < zmean[i] < left_front_max_z:
                    I_new.append(I[i])
                    J_new.append(J[i])
                    K_new.append(K[i])
...
if __name__ == '__main__':
    json_path = r'E:\CarInsurance\car_models_json_wd'
...
# for element in image_list[:1]:
    ...
    with open(os.path.join(json_path, elements)) as json_file:
        ...
        #     [-0.8, -0.6]]
        # point = [-0.2, 0.3]
        #### the back car insurance
        left_front_min_x = -0.9
        left_front_max_x = 0.9

        left_front_max_y = -2.1
        left_front_min_y = -4.0

        left_front_max_z = -0.1
        left_front_min_z = -1.0

        #### the front car insurance
        left_front_min_x = -0.9
        left_front_max_x = 0.9

        left_front_max_y = 3.0
        left_front_min_y = 2.2
