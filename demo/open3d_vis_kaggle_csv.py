...
def open_3d_vis(start_vis_index, valid_df, train_df, train_img_dir, car_model_dir):
    """
    ...
    valid_df = {imgID: str2coords(s, names=['pitch', 'yaw', 'roll', 'x', 'y', 'z','carid_or_score']) for imgID, s in
                  zip(valid_df['ImageId'], valid_df['PredictionString'])}
    idx = 0
    for key in valid_df.keys():
        idx += 1
        if idx > start_vis_index:
            pred = valid_df[key]
            img_name = key
            print('Visualising: %s' % (str(idx) + "__" + img_name))
            img = cv2.imread(os.path.join(train_img_dir, img_name + '.jpg'))
            # plt.figure()
            plt.ion()
            plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
            plt.title(str(idx) + "__" + img_name)
            plt.draw()
            plt.show()
            ...
            car_names = ['aodi-a6'] * len(pred)  # We use only one car model
            trans_pred_world = np.array([[x['x'], x['y'], x['z']] for x in pred])
            euler_angle = np.array([[x['pitch'], x['yaw'], x['roll']] for x in pred])
...
if __name__ == '__main__':
    ...
    valid_pred_csv = r'all_yihao069e100s5070_resume92Dec24-08-50-226141a3d1_valid_ep99_validation_images_conf_0.1.csv'
    ...
    valid_df = pd.read_csv(r'E:\DATASET\pku-autonomous-driving/cwx_data/' +valid_pred_csv)
