...
def pose_similarity(dt, gt, relative=True):
    ...
    dt_car_trans = np.array([dt['x'], dt['y'], dt['z']]).transpose()
    gt_car_trans = np.array([gt['x'], gt['y'], gt['z']]).transpose()

    dis_trans_abs = np.linalg.norm(np.tile(dt_car_trans[:, None, :], [1, gt_num, 1]) -
                               np.tile(gt_car_trans[None, :, :], [dt_num, 1, 1]), axis=2)

    if relative:
        distance_gt = np.sqrt(np.sum(gt_car_trans**2, axis=1))
        dis_trans_relative = dis_trans_abs / distance_gt[None, :]
    ...
    diff = abs(1 - np.sum(np.square(np.tile(q1[:, None, :], [1, gt_num, 1]) - np.tile(q2[None, :, :], [dt_num, 1, 1])),
                          axis=2) / 2.0)
...
class Detect3DEval(object):
    ...
    def __init__(self, args, train_df, valid_df):
        ...
        self.image_list = np.unique(self.train_df.ImageId)
        assert len(np.unique(self.valid_df.ImageId)) == len(self.image_list)

        self.params = Params(simType='3dpose')
    ...
    def _prepare(self):
        ...
        for image_name in self.image_list:
            self._gts[image_name] = self.train_df[self.train_df.ImageId == image_name]
            self._dts[image_name] = self.valid_df[self.valid_df.ImageId == image_name]
    ...
    def evaluate(self):
        ...
        # we also return the result for All
        return self.evalImgs[0]
...
class Params(object):
    ...
    def set_det_params(self):
        ...
        self.transThrs = np.linspace(0.1, 0.01, np.round((0.1 - 0.01) / .01) + 1, endpoint=True)

        self.criterion_num = len(self.transThrs)
        assert self.criterion_num == len(self.oriThrs)

        self.recThrs = np.linspace(.0, 1.00, np.round((1.00 - .0) / .01) + 1, endpoint=True)
        # from loss to strict criterion
        self.criteria = [Criterion(self.transThrs[i],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--validation_prediction', default='/data/Kaggle/wudi_data/work_dirs/cyh_Nov27-14-16-45.csv',
                        help='results')
    parser.add_argument('--gt_prediction', default='/data/Kaggle/pku-autonomous-driving/train.csv', help='ground truth')
    parser.add_argument('--car_conf_thres', default=0.5, help='the confidence threshold for car')
    args = parser.parse_args()

    # validation_prediction = '../input/autonomous-driving-validation-data/prediction_for_validation_data.csv'
    validation_prediction = args.validation_prediction
    valid_df = pd.read_csv(validation_prediction)
    expanded_valid_df = expand_df(valid_df, ['pitch', 'yaw', 'roll', 'x', 'y', 'z', 'Score'])
    expanded_valid_df = expanded_valid_df[expanded_valid_df.Score > args.car_conf_thres]
    valid_df = valid_df.fillna('')

    train_df = pd.read_csv(args.gt_prediction)
    ...
    det_3d_metric = Detect3DEval(args, expanded_train_df, expanded_valid_df)
